...
antigen () {
	...
	[[ "$ZSH_EVAL_CONTEXT" =~ "toplevel:*" || "$ZSH_EVAL_CONTEXT" =~ "cmdarg:*" ]] && source "/home/inet/antigen.zsh" && eval antigen $@
	return 0
}
async () {
	async_init
...
spaceship_gleam () {
	...
	spaceship::section::v4 --color "$SPACESHIP_GLEAM_COLOR" --prefix "$SPACESHIP_GLEAM_PREFIX" --suffix "$SPACESHIP_GLEAM_SUFFIX" --symbol "$SPACESHIP_GLEAM_SYMBOL" "v$gleam_version"
}
...
spaceship_package::cargo () {
	...
	echo "$pkgid" | grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} -q "error:" && return
	echo "${pkgid##*\#}"
}
...
spaceship_package::gradle () {
	...
	$gradle_exe properties --no-daemon --console=plain -q 2> /dev/null | grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} "^version:" | awk '{printf $2}'
}
...
spaceship_swift () {
	...
	then
		if swiftenv version | grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} ".swift-version" > /dev/null
		then
			swift_version=$(swiftenv version | sed 's/ .*//') 
		fi
	fi
...
spaceship_uv () {
	...
	[[ -z $py_version ]] && return
	spaceship::section --color "$SPACESHIP_UV_COLOR" --prefix "$SPACESHIP_UV_PREFIX" --suffix "$SPACESHIP_UV_SUFFIX" --symbol "$SPACESHIP_UV_SYMBOL" "$py_version"
}
spaceship_venv () {
...
spaceship_xcode () {
	...
	then
		if xcenv version | grep --color=auto --exclude-dir={.bzr,CVS,.git,.hg,.svn,.idea,.tox,.venv,venv} ".xcode-version" > /dev/null
		then
			xcode_path=$(xcenv version | sed 's/ .*//') 
		fi
	fi
...
if ! (unalias rg 2>/dev/null; command -v rg) >/dev/null 2>&1; then
  ...
  if [[ -n $ZSH_VERSION ]]; then
    ARGV0=rg /home/inet/.local/share/claude/versions/2.1.85 "$@"
  elif [[ "$OSTYPE" == "msys" ]] || [[ "$OSTYPE" == "cygwin" ]] || [[ "$OSTYPE" == "win32" ]]; then
    ARGV0=rg /home/inet/.local/share/claude/versions/2.1.85 "$@"
  elif [[ $BASHPID != $$ ]]; then
    exec -a rg /home/inet/.local/share/claude/versions/2.1.85 "$@"
  else
    (exec -a rg /home/inet/.local/share/claude/versions/2.1.85 "$@")
  fi
}
fi
export PATH=/home/inet/.local/bin\:/home/inet/.local/bin\:/usr/local/sbin\:/usr/local/bin\:/usr/bin\:/usr/bin/site_perl\:/usr/bin/vendor_perl\:/usr/bin/core_perl\:/home/inet/.antigen/bundles/robbyrussell/oh-my-zsh/lib\:/home/inet/.antigen/bundles/denysdovhan/spaceship-prompt\:/home/inet/.antigen/bundles/robbyrussell/oh-my-zsh/plugins/git\:/home/inet/.antigen/bundles/robbyrussell/oh-my-zsh/plugins/tmux\:/home/inet/.antigen/bundles/robbyrussell/oh-my-zsh/plugins/command-not-found\:/home/inet/.antigen/bundles/zsh-users/zsh-syntax-highlighting
